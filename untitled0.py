...
# The PDF file path, protein chain and RNA chain will be used for the calculation which are initialized during variable declaration
    ...
    def f6_2_protein_protein_contact2(self, dist_cutoff=5):
        ...
        b1 = [x.strip() in pal1 for x in int_df1['prot_res']]
        temp1 = int_df1[b1]
        b2 = [x.strip() in pal1 for x in temp1['prot_res']]
        df_inter = temp1[b2]
        return df_inter
...
#        numcols = len(df1.columns)
        for i in df1.index:
            ...
            idx2 = na_param[(str(df1['prot2_atm'][i]).strip() == na_param['Atm_name']) & (str(df1['prot2_res'][i]).strip() == na_param['Res_name'])].index
            if (len(idx2) == 0 ):
                # print (list(df1.columns))
...
inst2 = interaction_energy('1j1u.pdb')
h1 = inst2.f1_header()
...
df_inter.to_csv('atom_interaction.csv')
energy_df = inst2.f7_energy2(df_inter,aa_param,na_param)
energy_df.to_csv('interaction_energy.csv')
